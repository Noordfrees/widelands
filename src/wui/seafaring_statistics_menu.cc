...
SeafaringStatisticsMenu::SeafaringStatisticsMenu(InteractivePlayer& plr,
   ...
   : UI::UniqueWindow(&plr, "seafaring_statistics", &registry, 375, 375, _("Seafaring Statistics")),
     ...
     kaper_btn_(&filter_box_,
               ...
               kButtonSize,
               UI::ButtonStyle::kWuiSecondary,
               status_to_image(ShipFilterStatus::kKapers)),
     battle_btn_(&filter_box_,
               "filter_ship_battle",
               0,
               0,
               kButtonSize,
               kButtonSize,
               UI::ButtonStyle::kWuiSecondary,
               status_to_image(ShipFilterStatus::kBattleships)),
	...
	filter_box_.add(&kaper_btn_);
	filter_box_.add(&battle_btn_);
	...
	kaper_btn_.sigclicked.connect([this]() { filter_ships(ShipFilterStatus::kKapers); });
	battle_btn_.sigclicked.connect([this]() { filter_ships(ShipFilterStatus::kBattleships); });
...
SeafaringStatisticsMenu::status_to_string(SeafaringStatisticsMenu::ShipFilterStatus status) const {
	...
	case SeafaringStatisticsMenu::ShipFilterStatus::kKapers:
		return pgettext("ship_state", "Auf Kaperfahrt");  // NOCOM translation needed
	case SeafaringStatisticsMenu::ShipFilterStatus::kBattleships:
		return pgettext("ship_state", "Battleship");
...
SeafaringStatisticsMenu::status_to_image(SeafaringStatisticsMenu::ShipFilterStatus status) const {
	...
	case SeafaringStatisticsMenu::ShipFilterStatus::kRoaming:
		...
		break;
	case SeafaringStatisticsMenu::ShipFilterStatus::kKapers:
		filename = "images/wui/stats/ship_stats_kaper.png";
		break;
	case SeafaringStatisticsMenu::ShipFilterStatus::kBattleships:
		filename = "images/wui/stats/ship_stats_battleship.png";
		break;
	case SeafaringStatisticsMenu::ShipFilterStatus::kShipping:
...
SeafaringStatisticsMenu::create_shipinfo(const Widelands::Ship& ship) const {
	...
	case Widelands::Ship::ShipStates::kTransport:
		if (ship.get_destination()) {
			if (ship.get_fleet()->get_schedule().is_busy(ship)) {
				status = ShipFilterStatus::kShipping;
			} else {
				status = ShipFilterStatus::kRoaming;
			}
		} else {
	...
	case Widelands::Ship::ShipStates::kExpeditionColonizing:
		...
		break;
	case Widelands::Ship::ShipStates::kKaper:
		status = ShipFilterStatus::kKapers;
		break;
	case Widelands::Ship::ShipStates::kBattleship:
		status = ShipFilterStatus::kBattleships;
		break;
...
bool SeafaringStatisticsMenu::handle_key(bool down, SDL_Keysym code) {
	if (down) {
		...
		case SDLK_7:
			if (code.mod & KMOD_ALT) {
				filter_ships(ShipFilterStatus::kKapers);
				return true;
			}
			return false;
		...
		case SDLK_8:
			if (code.mod & KMOD_ALT) {
				filter_ships(ShipFilterStatus::kBattleships);
				return true;
			}
			return false;
...
void SeafaringStatisticsMenu::filter_ships(ShipFilterStatus status) {
	...
	case ShipFilterStatus::kRoaming:
		...
		break;
	case ShipFilterStatus::kKapers:
		toggle_filter_ships_button(kaper_btn_, status);
		break;
	case ShipFilterStatus::kBattleships:
		toggle_filter_ships_button(battle_btn_, status);
		break;
	case ShipFilterStatus::kAll:
		...
		battle_btn_.set_perm_pressed(false);
		kaper_btn_.set_perm_pressed(false);
		break;
	}
	fill_table();
}
...
void SeafaringStatisticsMenu::set_filter_ships_tooltips() {
	...
	kaper_btn_.set_tooltip(as_tooltip_text_with_hotkey(
	   /** TRANSLATORS: Tooltip in the ship statistics window. NOCOM needs translation */
	   _("Kaper zeigen"), pgettext("hotkey", "Alt+7")));
	battle_btn_.set_tooltip(as_tooltip_text_with_hotkey(
	   /** TRANSLATORS: Tooltip in the ship statistics window */
	   _("Show battleships"), pgettext("hotkey", "Alt+8")));
}
